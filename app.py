...
# Get user input for PICO
p_input = st.text_input("Patient, Population, or Problem")
i_input = st.text_input("Intervention")
c_input = st.text_input("Comparison")
o_input = st.text_input("Outcome")

user_input = f"{p_input} {i_input} {c_input} {o_input}"

# Search for articles using Pubmed API
if st.button("Search with EBPcharlie"):
    if not user_input.strip():
        st.error("Please enter the PICO components to search for articles.")
    else:
